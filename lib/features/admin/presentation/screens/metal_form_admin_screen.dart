// lib/features/admin/presentation/screens/metal_form_admin_screen.dart
...
class MetalFormAdminScreen extends ConsumerStatefulWidget {
  const MetalFormAdminScreen({super.key});

  @override
  ConsumerState<MetalFormAdminScreen> createState() =>
      _MetalFormAdminScreenState();
}

class _MetalFormAdminScreenState extends ConsumerState<MetalFormAdminScreen> {
  List<MetalFormRecord>? _records;
  bool _loading = true;
  String? _error;
  ...
  Future<void> _load() async {
    ...
    try {
      final response =
          await _db.from('metal_forms').select().order('name');
      if (mounted) {
        setState(() {
          _records = response
              .map((e) => MetalFormRecord.fromJson(e as Map<String, dynamic>))
              .toList();
          _loading = false;
        });
      }
  ...
  Future<void> _showAddDialog() async {
    ...
    try {
      await _db.from('metal_forms').insert({
        'name': ctrl.text.trim(),
        'is_active': true,
      });
      ref.invalidate(metalFormsProvider);
      await _load();
    } catch (e) {
      if (mounted) {
  ...
  Future<void> _showEditDialog(MetalFormRecord record) async {
    ...
    if (confirmed != true || ctrl.text.trim().isEmpty) return;
    try {
      await _db.from('metal_forms').update({'name': ctrl.text.trim()}).eq('id', record.id);
      ref.invalidate(metalFormsProvider);
      await _load();
    } catch (e) {
      if (mounted) {
  ...
  Future<void> _toggleActive(MetalFormRecord record) async {
    try {
      await _db
          .from('metal_forms')
  ...
  Future<void> _delete(MetalFormRecord record) async {
    final confirmed = await showDialog<bool>(
      context: context,
      builder: (ctx) => AlertDialog(
        backgroundColor: AppColors.backgroundCard,
    ...
    try {
      await _db.from('metal_forms').delete().eq('id', record.id);
      ref.invalidate(metalFormsProvider);
      await _load();
    } catch (e) {
      if (mounted) {
