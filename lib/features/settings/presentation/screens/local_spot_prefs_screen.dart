...
/// User preference screen for selecting local spot price retailers.
///
...
class LocalSpotPrefsScreen extends ConsumerStatefulWidget {
  final bool embedded;

  const LocalSpotPrefsScreen({super.key, this.embedded = false});

  @override
  ConsumerState<LocalSpotPrefsScreen> createState() =>
      _LocalSpotPrefsScreenState();
}

class _LocalSpotPrefsScreenState extends ConsumerState<LocalSpotPrefsScreen> {
  List<_RetailerOption> _options = [];
  ...
  Future<void> _load() async {
    try {
      ...
      // Pre-select: no current selections (legacy screen — use UserRetailerPrefsScreen instead)
  ...
  Future<void> _save() async {
    ...
    try {
      // Legacy screen — no-op save (use UserRetailerPrefsScreen instead)
      await Future.value();
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Local spot preferences saved'),
  ...
  Widget build(BuildContext context) {
    ...
    return AppScaffold(title: 'Local Spot Retailer Settings', body: body);
  }
  ...
  Widget _buildBody() {
    ...
    if (_options.isEmpty) {
      return const Padding(
        ...
        child: Text(
          'No local spot retailers configured yet.',
